...
#pragma region
			do
			{
				...
				h = FuncGenerator.GenerateRandomHashFunc2(s_M);
...
#pragma region
			...
			for (int j = 0; j < s_M; j++)
			{
				...
				do
				{
					hj = FuncGenerator.GenerateRandomHashFunc2(T[j].sj);
...
#pragma endregion Save all not Null elements from T[j] into List l
			...
			do
			{
				hj = FuncGenerator.GenerateRandomHashFunc2(sj);
